...
void Feature::calcFrequency()
{
    ...
    //double fps = PtrHandler->getVideoFPS();
...
void Feature::calc(const Region& region, const Mat& frame, char LR)
{
    mROI = frame(region.rect);
    cvtColor(mROI, mGray, CV_BGR2GRAY);
    const Mat& mask = PtrHandler->getDetector(LR).getExtractor().getMask();
...
{//返回一个1行len列的行矩阵，依次包括如下顺序的数据类型
//Mat_是opencv内置的类型，继承于Mat，增加了一些便捷操作
...
void FeatureAnalyzer::targetUpdate(map<int, Target>& targets, char LR)
{//目标的更新
    ...
    for (map<int, Target>::iterator it = targets.begin(); it != targets.end(); it++) {
        Target& target = it->second;
        if (target.type != Target::TARGET_LOST) {
            target.feature.calc(target.region, mFrame, LR);
        }
    }
}

void FeatureAnalyzer::analyze(const Mat& frame, map<int, Target>& targets, char LR)
{
    mFrame = frame;
    
    targetUpdate(targets, LR);
    
    Mat temp;
    mFrame.copyTo(temp);
    for (map<int, Target>::iterator it = targets.begin(); it != targets.end(); it++) 
    {
        rectangle(temp, it->second.region.rect, Scalar(0, 0, 255));
        //绿色方框圈中图片中可疑的目标
    }
    switch (LR)
    {
        case 'L':
            namedWindow("L_frame",  CV_WINDOW_NORMAL);
            imshow("L_frame", temp);
            break;
        case 'R':
            namedWindow("R_frame",  CV_WINDOW_NORMAL);
            imshow("R_frame", temp);
            break;
        default:
            cout << "analyze ERROR LR" << endl;
    }
